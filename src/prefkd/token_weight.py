...
def calculate_probability_differences(
    ...
    for i in tqdm(range(0, len(prompts_1), batch_size), desc=desc, mininterval=1.0, ncols=80):
        ...
        tokenized_responses = tokenizer(
            batch_responses, return_tensors="pt", padding=True, add_special_tokens=False
        )
        ...
        for j in range(len(batch_prompts_1)):
            ...
            prompt_ids_1 = tokenized_prompts_1.input_ids[j][
                tokenized_prompts_1.input_ids[j] != tokenizer.pad_token_id
            ]
            prompt_mask_1 = tokenized_prompts_1.attention_mask[j][
                tokenized_prompts_1.input_ids[j] != tokenizer.pad_token_id
            ]

            # Remove padding from prompt 2
            prompt_ids_2 = tokenized_prompts_2.input_ids[j][
                tokenized_prompts_2.input_ids[j] != tokenizer.pad_token_id
            ]
            prompt_mask_2 = tokenized_prompts_2.attention_mask[j][
                tokenized_prompts_2.input_ids[j] != tokenizer.pad_token_id
            ]

            # Remove padding from response
            response_ids = tokenized_responses.input_ids[j][
                tokenized_responses.input_ids[j] != tokenizer.pad_token_id
            ]
            response_mask = tokenized_responses.attention_mask[j][
                tokenized_responses.input_ids[j] != tokenizer.pad_token_id
            ]
        ...
        padded_input_ids_1 = [
            F.pad(ids, (0, max_len_1 - len(ids)), value=tokenizer.pad_token_id)
            for ids in combined_input_ids_1
        ]
        padded_attention_mask_1 = [
            F.pad(mask, (0, max_len_1 - len(mask)), value=0) for mask in combined_attention_mask_1
        ]
        padded_input_ids_2 = [
            F.pad(ids, (0, max_len_2 - len(ids)), value=tokenizer.pad_token_id)
            for ids in combined_input_ids_2
        ]
        padded_attention_mask_2 = [
            F.pad(mask, (0, max_len_2 - len(mask)), value=0) for mask in combined_attention_mask_2
        ]
        ...
        for j in range(len(batch_prompts_1)):
            prompt_length_1 = (
                len(tokenizer.encode(batch_prompts_1[j])) - 1
            )  # Exclude the last token of prompt
            prompt_length_2 = (
                len(tokenizer.encode(batch_prompts_2[j])) - 1
            )  # Exclude the last token of prompt
            ...
            for k in range(response_length):
                ...
                assert actual_next_token_id_1 == actual_next_token_id_2, (
                    "Response tokens should be the same for both models"
                )
...
def process_dataset_shard(gpu_id, input_file, model_name_1, model_name_2, data_shard, batch_size=8):
...
def main():
    ...
    parser.add_argument(
        "--positive_model_name",
        type=str,
        default="/home/ngannt61/big/output/dpo_Llama1b_full_06-13_12-26/Llama1b",
        help="Path to the first model.",
    )
    parser.add_argument(
        "--negative_model_name",
        type=str,
        default="/home/ngannt61/big/output/dpo_Llama1b_full_reverse_06-15_20-29/Llama1B",
