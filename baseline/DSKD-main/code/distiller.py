...
class Distiller(nn.Module):
    def __init__(self, args, device):
        ...
        if args.teacher_to_student_token_mapping is not None:
            ...
            log_rank(
                f"Load teacher-to-student token mapping from {args.teacher_to_student_token_mapping}"
            )
        ...
        if args.teacher_to_student_id_mapping is not None:
            ...
            log_rank(
                f"Load teacher-to-student id mapping from {args.teacher_to_student_id_mapping}"
            )
            ...
            for stu_id in self.stu2tea_id_mapping:
                self.stu2tea_id_mapping[stu_id] = self.stu2tea_id_mapping[stu_id][
                    :max_align_num
                ] + [self.stu2tea_id_mapping[stu_id][-1]] * max(
                    0, max_align_num - len(self.stu2tea_id_mapping[stu_id])
                )

            self.tea2stu_id_mapping = torch.LongTensor(list(self.tea2stu_id_mapping.values())).to(
                device
            )
            self.stu2tea_id_mapping_tea = torch.LongTensor(
                list(self.stu2tea_id_mapping.values())
            ).to(device)
            self.stu2tea_id_mapping_stu = torch.LongTensor(list(self.stu2tea_id_mapping.keys())).to(
                device
            )
    ...
    def add_distiller_args(parser):
        ...
        group.add_argument(
            "--projector-config-path", type=str, default=None, help="path to projector_config.json"
        )
        group.add_argument(
            "--projector-path", type=str, default=None, help="path to pretrained projector"
        )
        group.add_argument(
            "--projector-lr", type=float, default=0.001, help="learning rate only for projection"
        )
        group.add_argument(
            "--pretrained-projector", type=str, default=None, help="pretrained projector name"
        )
        group.add_argument(
            ...
            if projector_config[projector_name]["enabled"]:
                ...
                for i in range(len(structure) - 1):
                    if isinstance(structure[i], int) and isinstance(structure[i + 1], int):
                        ...
                        self.projectors[projector_name].append(name_dict[structure[i + 1]])
    ...
    def load_existing_projectors(self):
        ...
        if os.path.exists(projector_path):
            ...
            for key in self.projectors:
                try:
                    state_dict = {
                        n.split(".", 1)[1]: projector_params[n]
                        for n in projector_params
                        if n.startswith(key)
                    }
        ...
        if hasattr(self, "projectors"):
            if self.args.projector_lr:
                ...
                optimizer.add_param_group(
                    {
                        "params": [
                            p
                            for b in self.projectors
                            if b not in pretrained_proj
                            for p in self.projectors[b].parameters()
                        ],
                ...
                optimizer.add_param_group(
                    {
                        "params": [
                            p for b in self.projectors for p in self.projectors[b].parameters()
                        ],
                    }
                )
        return optimizer
