...
def KL_stable(y_s, y_t, T=1):
    # Use log_softmax for the student predictions (log probabilities)
    p_s = F.log_softmax(y_s / T, dim=-1)
    # Use softmax for the teacher predictions (probabilities)
    p_t = F.softmax(y_t / T, dim=-1)

    # Use PyTorch's built-in KLDivLoss
    # reduction='batchmean' averages the loss over the batch
    # log_target=False means the target (p_t) is probabilities, not log-probabilities
    loss_fct = nn.KLDivLoss(reduction="batchmean", log_target=False)

    return loss_fct(p_s, p_t)
...
class MULTI_LEVEL_OT(VariousDivergence):
    ...
    def forward(
        ...
        loss = self.compute_cross_entropy_loss(outputs.logits, output_data["label"])[0]
    ...
    def __get_start_and_size_answers(self, answer_tensors):
        ...
        print(
            f"Answer tensors shape: {answer_tensors.shape if hasattr(answer_tensors, 'shape') else 'No shape'}"
        )
    ...
    def compute_multi_level_ot_distillation_loss(
        ...
        # ===================== CHECK 1: RAW LOGITS =====================
        if torch.isnan(student).any() or torch.isinf(student).any():
            print("!!! NaN/inf DETECTED IN RAW STUDENT LOGITS !!!")
        if torch.isnan(teacher).any() or torch.isinf(teacher).any():
            print("!!! NaN/inf DETECTED IN RAW TEACHER LOGITS !!!")
        # ===============================================================

        # Print tensor shapes and dtypes for debugging
        print(
            f"Student logits shape: {student.shape}, dtype: {student.dtype}, min: {student.min().item()}, max: {student.max().item()}"
        )
        print(
            f"Teacher logits shape: {teacher.shape}, dtype: {teacher.dtype}, min: {teacher.min().item()}, max: {teacher.max().item()}"
        )
        ...
        if student.size(1) == 1 or teacher.size(1) == 1:
            ...
            print(
                f"After processing for single token case - Student: {student.shape}, Teacher: {teacher.shape}"
            )
        else:
        ...
        # ===================== CHECK 2: AFTER SOFTMAX =====================
        if torch.isnan(student).any() or torch.isinf(student).any():
            print("!!! NaN/inf DETECTED IN STUDENT TENSOR AFTER SOFTMAX !!!")
        if torch.isnan(teacher).any() or torch.isinf(teacher).any():
            print("!!! NaN/inf DETECTED IN TEACHER TENSOR AFTER SOFTMAX !!!")
        # ==================================================================
        ...
        # ===================== CHECK 3: AFTER FIRST SORT ==================
        if torch.isnan(student).any() or torch.isinf(student).any():
            print("!!! NaN/inf DETECTED IN STUDENT TENSOR AFTER FIRST SORT !!!")
        # ==================================================================
        ...
        # ===================== CHECK 4: AFTER IMPROVED_SORT ===============
        if torch.isnan(student).any() or torch.isinf(student).any():
            print("!!! NaN/inf DETECTED IN STUDENT TENSOR AFTER IMPROVED_SORT !!!")
        # ==================================================================
        ...
        # multi_level_ot_loss = mean_distillation_loss
        ...
        print(
            f"L_HAD: {mean_distillation_loss.item()}, L_SL: {kl_loss.item()}, L_SD: {sinkhorn_loss.item()}"
        )
